...
DT=["test/agent/priority-preserving-context-cap.test.ts","test/agent/qwen-provider.test.ts","test/agent/codex-sdk-provider.test.ts",
...
MUT = [
 ("Q1 qwen cap call removed (raw argv prompt)", Q, "args.push('--append-system-prompt', capQwenAppendSystemPrompt(effectivePrompt));", "args.push('--append-system-prompt', effectivePrompt);", "daemon", DT),
 ...
 ("Q6 closing tag via indexOf instead of lastIndexOf", H, "  const close = text.lastIndexOf(SESSION_IDENTITY_BLOCK_CLOSE_TAG);", "  const close = text.indexOf(SESSION_IDENTITY_BLOCK_CLOSE_TAG, bodyStart);", "daemon", DT),
 ...
 ("S4 validator counts UTF-16 units", S, "  if (Array.from(normalized).length > sessionIdentityMaxChars(scope)) return 'identity_content_too_large';", "  if (normalized.length > sessionIdentityMaxChars(scope)) return 'identity_content_too_large';", "daemon", DT),
...
for label, path, old, new, project, tests in MUT:
    ...
    try:
        ...
        k = r.returncode != 0
        fails = sorted({l.split('> ')[-1][:100] for l in r.stdout.splitlines() if 'FAIL ' in l})
