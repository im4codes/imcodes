...
function makeMockProvider() {
  ...
  return {
    provider: {
      ...
      endSession: vi.fn(),
      onDelta: (cb: (sid: string, d: MessageDelta) => void) => { deltaCb = cb; },
      onComplete: (cb: (sid: string, m: AgentMessage) => void) => { completeCb = cb; },
      onError: (cb: (sid: string, e: ProviderError) => void) => { errorCb = cb; },
