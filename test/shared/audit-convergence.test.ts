...
describe('audit convergence contract', () => {
  ...
  it('carries every convergence rule in one locale-invariant body', () => {
    ...
    expect(contract.evidence.auditorExecution).toMatch(/never full project\/build\/coverage\/e2e/);
    expect(contract.evidence.suspicion).toMatch(/do not REWORK merely to request that check/);
    ...
    expect(contract.evidence.integrity).toMatch(/authoritative accepted implementer report/);
