...
Зарегистрируйте другую машину как ограниченный controlled node, не превращая её в полноценный IM.codes source server. Наберите `^` для автодополнения цели или вставьте `^^(name)`; авторизованные агенты смогут выполнять на узле scoped remote commands, передавать отдельные файлы и вызывать типизированные Computer Use tools. Владелец включает и отзывает право выполнения, controlled nodes не попадают в обычные списки server/session, а каждая машина получает отдельные учётные данные. Ссылка на скачивание истекает, но новый установщик можно сохранить и повторно использовать для регистрации нескольких машин.

### Удалённое управление рабочим столом

Поддерживаемые управляемые узлы отдают непрерывный удалённый рабочий стол по H.264 авторизованным владельцам и участникам. Браузер и нативный worker сначала согласуют прямой маршрут WebRTC и используют TURN только как запасной вариант; видео и данные мыши/клавиатуры не проходят через сервер приложения. Режим управления включён по умолчанию, разрешено несколько авторизованных операторов, и каждый зритель может самостоятельно переключиться в режим просмотра. Вкладки дисплеев дают контекстное меню (правый клик или долгое нажатие) для 720p, 1080p, 1440p и 4K; на мобильном есть навигация щипком/перетаскиванием и режим виртуальной мыши с панорамированием у краёв, левой/средней/правой кнопками и колесом.

Для Windows используется подписанный предсобранный worker: управляемым узлам не нужны компиляторы и медиазависимости. **Сейчас поддерживается Windows, macOS и Linux — скоро.**
