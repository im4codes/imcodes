...
IM.codes 也支持围绕同一个 agent 工作区进行真人协作。你可以把当前标签页、子会话或整台 source server 分享给其他用户，并选择 `viewer` 或 `participant` 角色。

- **双人或多人 agent 编程。** 两个人可以一起看同一个实时 coding session；任务需要更多上下文或 review 时，也可以让多人围绕同一 server 协作。
- **有作用域的分享。** 只分享当前标签页/子会话，适合聚焦交接；分享整台 source server，则适合协作者在相关 session 间切换。
- **Viewer 与 participant 分权。** Viewer 可以查看有作用域的状态并参与讨论，但不能驱动 agent；participant 可以向覆盖范围内的具体标签页发送 prompt。
- **可审计的真人身份。** 通过 shared access 发送的消息会标注真人 actor 和角色，时间线能看出是谁下达了指令。
- **可随时撤销。** UI 中可以管理共享用户、降级或撤销权限；participant 权限会显示显式信任提示，因为 prompt 仍可能影响未沙箱化的 agent。
...
- **实现前先做规格审计。** 可选的提案/规格 audit-repair 使用常规 Team/P2P 组合流程（默认 `audit>review>plan`），并读取权威 JSON，而不是盲信聊天摘要。
...
把实时 session 分享给另一个人做 pair programming，或者邀请多人进入有作用域的 server 工作区做 mob-style agent supervision。基于角色的访问把只读 viewer 和可发送 prompt 的 participant 分开，shared message 会携带 actor 标识，协作过程可追踪。
