...
**给 AI agent 的 IM。共享记忆、托管 MCP 工具、受监督执行，以及跨模型审计。**
...
[IM.codes](https://im.codes) 同时解决这两类问题。它让这些会话在手机或网页上始终可达：打开终端、检查文件和 Git 变更、在其他设备上预览 localhost、在任务完成时收到通知，并继续调度多个 agent。它也把下文的「共享代理上下文与记忆」和「跨模型审计与 P2P 讨论」连在一起：可持久召回的是已完成工作的摘要记忆，P2P 讨论则是在代码落地前进行的结构化跨模型审阅。它不能让输出完美，但能减少单模型盲点，让复杂工作在更多审阅下更容易收敛。
...
- **记忆召回与来源。** `search_memory` 会在当前调用者绑定的记忆命名空间中搜索过往工作、项目历史、决策、偏好、bug、commit、部署和既往讨论上下文。结果包含紧凑摘要和 `projectionId`；当模型需要精确的历史指令、bug 细节、commit/部署上下文或来源证据时，`get_memory_sources` 可把相关命中展开成来源片段。
...
- **Cron 调度。** `cron_create`、`cron_list`、`cron_update` 和 `cron_delete` 管理未来的结构化发送任务，可用于提醒、周期性检查、委托 review 或定时 P2P follow-up，并支持目标、session、project、过期时间和时区字段。
...
单模型输出不应被盲目信任。P2P 讨论让多个 agent——跨不同 provider 和思维风格——在写代码之前就对同一代码库进行协作分析。每轮遵循可自定义的多阶段流程，每个 agent 读取所有前序贡献并在此基础上输出。不同模型捕获不同类别的问题：一个发现竞态条件，另一个指出遗漏的 migration，第三个质疑 API 设计。这种跨 provider 交叉审查能在实现前发现单个模型常漏掉的问题，减少返工。
