...
IM.codes는 coding agent를 위한, provider를 가로지르는 공유 메모리 레이어이자 관리형 MCP tool surface입니다. 완료된 작업을 재사용 가능한 컨텍스트로 축적하고, 적절한 기록을 이후 session에 주입하거나 recall합니다. Claude Code, Codex, Gemini CLI, GitHub Copilot, Cursor, OpenCode, OpenClaw, Qwen 등을 지원하며, 터미널, 파일 브라우징, Git 보기, localhost 미리보기, 알림, 멀티 에이전트 워크플로우, transport 기반 agent의 네이티브 스트리밍 출력도 함께 제공합니다. 세션 공유는 실행 중인 탭이나 전체 source server를 범위가 있는 협업 공간으로 바꾸어, 한 사람은 지켜보고 다른 사람은 participant로 prompt를 보낼 수 있게 합니다. OpenSpec Auto Deliver는 변경을 proposal/spec 감사부터 구현, 검증 힌트, Team 감사/재작업, 자동 모듈 채점, 최종 quality gate까지 진행할 수 있습니다. 내장된 Auto supervision은 완료된 턴을 판정하고, 자동 계속과 감사/재작업 루프까지 수행한 뒤 제어를 돌려줄 수 있습니다. Team 토론 기능 내장 — 여러 모델이 서로의 계획과 구현을 리뷰하고 감사하여, 단일 모델의 누락·맹점·편향을 효과적으로 줄입니다.
...
IM.codes는 같은 agent workspace를 중심으로 사람 간 협업도 지원합니다. 현재 탭, sub-session, 또는 전체 source server를 다른 사용자에게 공유하고 `viewer` 또는 `participant` 역할을 선택할 수 있습니다.

- **Agent pair/mob programming.** 두 사람이 같은 live coding session을 함께 볼 수 있고, 더 많은 맥락이나 리뷰가 필요하면 여러 사람이 같은 server를 중심으로 참여할 수 있습니다.
- **범위가 있는 공유.** 집중된 handoff에는 현재 탭/sub-session만 공유하고, 관련 session 사이를 이동해야 하면 전체 source server를 공유할 수 있습니다.
- **Viewer와 participant 역할 분리.** Viewer는 범위 내 상태를 보고 discussion에 참여할 수 있지만 agent를 구동하지 않습니다. Participant는 포함된 구체 탭에 prompt를 보낼 수 있습니다.
- **보이는 shared actor.** Shared access로 보낸 메시지는 사람 actor와 role이 표시되어 timeline에서 누가 지시했는지 확인할 수 있습니다.
- **회수 가능한 접근.** 공유 사용자는 UI에서 관리, 권한 낮춤, 회수할 수 있습니다. participant 접근에는 prompt가 sandbox되지 않은 agent에 영향을 줄 수 있다는 명시적 trust warning이 표시됩니다.
...
- **구현 전 spec audit.** 선택적 proposal/spec audit-repair는 일반 Team/P2P combo flow(기본 `audit>review>plan`)를 사용하며 chat summary가 아니라 authoritative JSON을 읽습니다.
...
### 협업 코딩
Live session을 다른 사람에게 공유해 pair programming을 하거나, 여러 사람을 범위가 있는 server workspace에 초대해 mob-style agent supervision을 할 수 있습니다. 역할 기반 접근은 읽기 전용 viewer와 prompt를 보낼 수 있는 participant를 분리하고, shared message에는 actor label이 붙어 협업 과정을 추적할 수 있습니다.
