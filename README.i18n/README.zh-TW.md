...
IM.codes 也支援圍繞同一個 agent 工作區進行真人協作。你可以把目前標籤頁、子 session 或整台 source server 分享給其他使用者，並選擇 `viewer` 或 `participant` 角色。

- **雙人或多人 agent 編程。** 兩個人可以一起看同一個即時 coding session；任務需要更多上下文或 review 時，也可以讓多人圍繞同一 server 協作。
- **有作用域的分享。** 只分享目前標籤頁/子 session，適合聚焦交接；分享整台 source server，則適合協作者在相關 session 間切換。
- **Viewer 與 participant 分權。** Viewer 可以查看有作用域的狀態並參與討論，但不能驅動 agent；participant 可以向覆蓋範圍內的具體標籤頁送出 prompt。
- **可審計的真人身份。** 透過 shared access 送出的訊息會標註真人 actor 和角色，時間線能看出是誰下達了指令。
- **可隨時撤銷。** UI 中可以管理共享使用者、降級或撤銷權限；participant 權限會顯示明確信任提示，因為 prompt 仍可能影響未沙箱化的 agent。
...
把即時 session 分享給另一個人做 pair programming，或者邀請多人進入有作用域的 server 工作區做 mob-style agent supervision。基於角色的存取把唯讀 viewer 和可送出 prompt 的 participant 分開，shared message 會攜帶 actor 標識，協作過程可追蹤。
