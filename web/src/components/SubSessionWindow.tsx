...
export function SubSessionWindow({
  ...
  return (
    <div ref={swipeBackRef} class="subsession-window" style={style} onMouseDown={onFocus}>
      ...
      {(lastUsage || activeThinkingTs || statusText || sessionInfo?.state === 'running' || sessionInfo?.state === 'idle' || sessionInfo?.planLabel || sessionInfo?.quotaLabel || sessionInfo?.quotaUsageLabel) && (
        <UsageFooter
