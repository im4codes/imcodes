...
describe('controlled-node remote install link', () => {
  it('asks the server for a remote-link ticket and returns a pasteable URL', async () => {
    apiFetch.mockResolvedValueOnce(ticketResponse({
      ...
      expiresAt: NOW + CONTROLLED_NODE_TICKET_TTL_MS[CONTROLLED_NODE_TICKET_DELIVERY.REMOTE_LINK],
    }));
    ...
    expect(link.expiresAt).toBe(
      NOW + CONTROLLED_NODE_TICKET_TTL_MS[CONTROLLED_NODE_TICKET_DELIVERY.REMOTE_LINK],
    );
