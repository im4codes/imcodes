...
describe('SessionTabs', () => {
  ...
  it('keeps owned tabs visible while rendering shared entries as marked peer tabs', () => {
    const onSelect = vi.fn();
    const onSelectSharedEntry = vi.fn();
    const sessions = makeSessions([{ name: 'owned_brain', role: 'brain', project: 'Owned' }]);
    const sharedEntry = {
      id: 'share-1',
    ...
    const sharedButton = screen.getByText('Shared Alpha').closest('button');
    expect(sharedButton).not.toBeNull();
    expect(sharedButton?.getAttribute('aria-selected')).toBe('true');

    fireEvent.click(screen.getByText('Owned').closest('button')!);
    expect(onSelect).toHaveBeenCalledWith('owned_brain');
    fireEvent.click(sharedButton!);
    expect(onSelectSharedEntry).toHaveBeenCalledWith(sharedEntry);
  });

  it('does not show the owned-session empty state when only shared entries exist', () => {
    render(
      <SessionTabs
        sessions={[]}
        activeSession={null}
        ...
        sharedEntries={[{
          ...
          role: 'viewer',
          status: 'active',
          targetLabel: 'Shared Only',
          target: { kind: 'server', serverId: 'shared-server' },
        }]}
        onSelectSharedEntry={vi.fn()}
        {...defaultProps}
      />,
    );

    expect(screen.queryByText('No active sessions')).toBeNull();
    expect(screen.getByText('Shared Only')).toBeDefined();
  });
