...
- **MANDATORY: No hardcoded strings for types/statuses/message names.** Always use shared constants or enums from `shared/`. Existing shared types: `shared/repo-types.ts` (repo message types), `shared/p2p-status.ts` (P2P run statuses), `shared/p2p-modes.ts` (P2P modes), `shared/cookie-names.ts` (cookie/CSRF constants).
