...
IM.codes is a three-tier system for remote-controlling AI coding agents via chat platforms:
...
- **MANDATORY: No hardcoded strings for types/statuses/message names.** Always use shared constants or enums from `shared/`. Existing shared types: `shared/repo-types.ts` (repo message types), `shared/p2p-status.ts` (P2P run statuses), `shared/p2p-modes.ts` (P2P modes). When web can't directly import `shared/`, define the mapping function once at the top of the file using Sets — NEVER inline string comparisons in multiple places.
