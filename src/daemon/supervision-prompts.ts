...
export function buildSupervisionTaskFinalizationContract(locale?: SupervisionUiLocale): string {
  ...
  const lines: Record<SupervisionUiLocale, string> = {
    en: `Task finalization contract (machine-checkable): first define topLevelTaskId, acceptance, and integrationBoundary; then split sliceId/ownerSession/ownedFiles/dependencies/sharedFiles. Each slice and top-level logical task MUST project fields {${fields}} and lifecycle ${states}. A slice ownerSession owns implementation, validation, ownedFiles, and a frozen revision/evidence handoff, then marks ready_for_integration without starting an audit; slice owners MUST NOT stage/commit/push. The orchestrator groups validated slices by topLevelTaskId/full feature, assigns one integrationOwnerSession, and that integration owner performs combination validation plus one overall cross-vendor matching audit before finalizing one coherent commit/push. Store integrationManifest with each slice classification/ownerSession/revision/validation evidence and ownedFiles. Only truly independent top-level tasks that can be separately built, rolled back, and shipped may split commits; same directory does not imply same task, and same-file/BUILD-graph coupling forbids partial commits. Never split by provider, test file, or small file fragments. If slices share a file, either include it in the integration task with each owner signed off or decouple the not-ready feature first; original owners must not commit another owner's files. Audit records attemptId + revision; integration_slice audit registration is forbidden; only one matching PASS for the SAME combined integration_task or independent_top_level revision may move the top-level task to passed/finalizing. Before matching PASS, stage/commit/push/finalization is absolutely forbidden. REWORK or old-attempt PASS never releases a newer revision, and a global/matching audit gate wins over disjoint-task independence. Before coherent commit: integrationManifest, ownedFiles, and staged diff manifest are mandatory; use explicit pathspecs only; directory pathspecs are allowed only when expanded staged paths exactly equal the PASS integration manifest. Compare git diff --cached --name-only against the manifest exact set; any missing, extra, UU/conflict, untracked-other-owner, unreviewed slice, or unrelated dirty staged path blocks finalization. ${forbiddenAdd} is forbidden; never stage ${forbiddenPrefixes}; never mix unrelated dirty files. Commit message/receipt must list topLevelTaskId, included slices, audit attempts/revisions, commitSha, pushResult, and pushRemoteRef; finalization failure blocks only that top-level task and must not mutate other task states.`,
    'zh-CN': `任务收尾合同（机器可检查）：每个 logical task 必须投影字段 {${fields}} 与生命周期 ${states}。slice ownerSession 负责实现、验证、ownedFiles，并冻结 revision/evidence 后直接交接且不发起审计，然后只标记 ready_for_integration；slice owner 不得 stage/commit/push。统筹者按 topLevelTaskId/完整功能汇总已验证 slices，指定单一 integrationOwnerSession；integration owner 完成组合验证与整体 matching audit 后，才做一个 coherent commit/push。integrationManifest 必须引用每个 slice classification/ownerSession/revision/validation evidence 和 ownedFiles。只有真正独立的顶层任务可拆 commit，禁止按 provider/测试文件/小文件碎片化提交。若 slices 共享文件，必须建立一个 integration task/commit，待所有 slice 验证交接并完成唯一整体 matching PASS 后再提交，原 owner 不得提交别人的文件。审计记录 attemptId + revision；禁止为 integration_slice 注册审计；只有合并后的 integration_task 或独立 independent_top_level 同一 revision 的唯一 matching PASS，才可使顶层任务进入 passed/finalizing。matching PASS 前绝对禁止 stage/commit/push/finalization。REWORK 或旧 attempt PASS 绝不释放新 revision；全局/匹配审计 gate 优先于不相交任务独立性。提交前必须有 ownedFiles；git add 只允许显式 pathspec；禁止 ${forbiddenAdd}；永不 stage ${forbiddenPrefixes}；不得混入无关 dirty files。每个任务独立 commit message、commitSha、pushResult、pushRemoteRef；收尾失败只阻塞该任务，不得篡改其他任务状态。`,
    'zh-TW': `任務收尾合同（機器可檢查）：每個 logical task 必須投影欄位 {${fields}} 與生命週期 ${states}。slice ownerSession 負責實作、驗證、ownedFiles，凍結 revision/evidence 後直接交接且不發起審計，然後只標記 ready_for_integration；slice owner 不得 stage/commit/push。統籌者按 topLevelTaskId/完整功能彙總已驗證 slices，指定單一 integrationOwnerSession；integration owner 完成組合驗證與整體 matching audit 後，才做一個 coherent commit/push。integrationManifest 必須引用每個 slice classification/ownerSession/revision/validation evidence 與 ownedFiles。只有真正獨立的頂層任務可拆 commit，禁止按 provider/測試檔/小檔案碎片化提交。若 slices 共享檔案，必須建立一個 integration task/commit，待所有 slice 驗證交接並完成唯一整體 matching PASS 後再提交，原 owner 不得提交別人的檔案。審計記錄 attemptId + revision；禁止為 integration_slice 註冊審計；只有合併後的 integration_task 或獨立 independent_top_level 同一 revision 的唯一 matching PASS，才可使頂層任務進入 passed/finalizing。matching PASS 前絕對禁止 stage/commit/push/finalization。REWORK 或舊 attempt PASS 絕不釋放新 revision；全域/匹配審計 gate 優先於不相交任務獨立性。提交前必須有 ownedFiles；git add 只允許明確 pathspec；禁止 ${forbiddenAdd}；永不 stage ${forbiddenPrefixes}；不得混入無關 dirty files。每個任務獨立 commit message、commitSha、pushResult、pushRemoteRef；收尾失敗只阻塞該任務，不得竄改其他任務狀態。`,
    es: `Contrato de finalización por tarea (verificable): cada tarea lógica proyecta {${fields}} y ciclo ${states}. La ownerSession de cada slice conserva implementación, validación y ownedFiles; congela revision/evidence y entrega sin auditoría, luego marca ready_for_integration; los slice owners NO hacen stage/commit/push. El orquestador agrupa slices validados por topLevelTaskId/función completa y asigna una sola integrationOwnerSession; ese owner ejecuta validación combinada y una auditoría global coincidente antes de un commit/push coherente. integrationManifest referencia classification/ownerSession/revision/evidencia de validación/ownedFiles de cada slice. Solo tareas top-level realmente independientes separan commits; nunca por provider, prueba o fragmentos pequeños. Si comparten archivo, crea tarea/commit de integración tras entregar todas las validaciones de slice y obtener el único matching PASS global y los owners originales no confirman archivos ajenos. La auditoría registra attemptId+revision; se prohíbe registrar auditoría para integration_slice; solo un PASS coincidente de la revision combinada integration_task o independent_top_level mueve la tarea superior a passed/finalizing. Antes de matching PASS, stage/commit/push/finalization están absolutamente prohibidos. REWORK o PASS viejo no libera revision nueva; un gate global/coincidente prevalece. Antes de commit: ownedFiles obligatorio; solo pathspec explícito; prohibido ${forbiddenAdd}; nunca stage ${forbiddenPrefixes}; no mezcles dirty files ajenos. Cada tarea tiene commit message, commitSha, pushResult y pushRemoteRef propios; un fallo bloquea solo esa tarea.`,
    ru: `Контракт финализации задачи (проверяемый): каждая logical task проецирует {${fields}} и цикл ${states}. slice ownerSession ведёт implementation, validation и ownedFiles, затем фиксирует revision/evidence и передаёт без аудита, после чего помечает ready_for_integration; slice owners НЕ делают stage/commit/push. Orchestrator группирует валидированные slices по topLevelTaskId/full feature и назначает один integrationOwnerSession; этот owner выполняет combination validation и общий matching audit перед одним coherent commit/push. integrationManifest ссылается на classification/ownerSession/revision/validation evidence/ownedFiles каждого slice. Только реально независимые top-level tasks разделяют commits; не по provider/test/small-file fragments. Общий файл => integration task/commit после передачи валидации всех slices и единственного общего matching PASS, исходные owners не коммитят чужие файлы. Audit хранит attemptId+revision; регистрация аудита для integration_slice запрещена; только один matching PASS объединённой revision integration_task или independent_top_level переводит top-level в passed/finalizing. До matching PASS stage/commit/push/finalization абсолютно запрещены. REWORK или старый PASS не освобождает новую revision; global/matching gate выше независимости. Перед coherent commit нужны integrationManifest и ownedFiles; только явный pathspec; запрещено ${forbiddenAdd}; никогда stage ${forbiddenPrefixes}; не смешивать чужие dirty files. У каждой top-level task свой commit message, commitSha, pushResult, pushRemoteRef; сбой блокирует только эту top-level task.`,
    ja: `タスク finalization 契約（機械検査可能）：各 logical task は {${fields}} とライフサイクル ${states} を投影します。slice ownerSession が implementation、validation、ownedFiles を担当し、revision/evidence を凍結して監査なしで引き渡した後 ready_for_integration を付けます；slice owner は stage/commit/push しません。orchestrator は topLevelTaskId/full feature ごとに検証済み slices をまとめ、単一 integrationOwnerSession を指定；その owner が combination validation と overall matching audit を行ってから coherent commit/push を1つ実行します。integrationManifest は各 slice の classification/ownerSession/revision/validation evidence/ownedFiles を参照します。本当に独立した top-level task だけ commit 分割可；provider/test/small-file fragments 単位は禁止。ファイル共有時は全 slice の検証引き渡しと唯一の overall matching PASS 後に integration task/commit を作成、元 owner は他者のファイルを commit しません。監査は attemptId+revision を記録；integration_slice の監査登録は禁止です；結合済み integration_task または独立 independent_top_level の同じ revision に対する唯一の matching PASS だけが top-level を passed/finalizing に進めます。matching PASS 前は stage/commit/push/finalization を絶対禁止。REWORK や旧 attempt PASS は新 revision を解放しません。global/matching gate が優先。coherent commit 前に integrationManifest と ownedFiles 必須；明示 pathspec のみ；${forbiddenAdd} 禁止；${forbiddenPrefixes} は never stage；無関係 dirty files 混入禁止。各 top-level task は独自 commit message/commitSha/pushResult/pushRemoteRef；失敗はその top-level task のみブロック。`,
    ko: `task finalization 계약(기계 검증 가능): 각 logical task는 {${fields}}와 생명주기 ${states}를 투영합니다. slice ownerSession은 구현, 검증, ownedFiles를 책임지고 revision/evidence를 동결해 감사 없이 인계한 뒤 ready_for_integration을 표시합니다; slice owner는 stage/commit/push하지 않습니다. orchestrator는 topLevelTaskId/full feature별로 검증된 slices를 묶고 단일 integrationOwnerSession을 지정합니다; 그 owner가 combination validation과 overall matching audit 후 하나의 coherent commit/push를 수행합니다. integrationManifest는 각 slice의 classification/ownerSession/revision/validation evidence/ownedFiles를 참조합니다. 진짜 독립 top-level task만 commit 분리 가능하며 provider/test/small-file fragments 단위 분리는 금지입니다. 파일을 공유하면 모든 slice 검증 인계와 단 하나의 overall matching PASS 후 integration task/commit을 만들며 원 owner는 다른 owner의 파일을 commit하지 않습니다. 감사는 attemptId+revision을 기록; integration_slice 감사 등록은 금지됩니다; 결합된 integration_task 또는 독립 independent_top_level의 같은 revision에 대한 단 하나의 matching PASS만 top-level을 passed/finalizing으로 이동합니다. matching PASS 전 stage/commit/push/finalization은 절대 금지입니다. REWORK나 old-attempt PASS는 새 revision을 해제하지 않으며 global/matching gate가 우선합니다. coherent commit 전 integrationManifest와 ownedFiles 필수; 명시 pathspec만; ${forbiddenAdd} 금지; ${forbiddenPrefixes} never stage; 무관한 dirty files 혼입 금지. 각 top-level task는 별도 commit message/commitSha/pushResult/pushRemoteRef; 실패는 해당 top-level task만 차단합니다.`,
  };
