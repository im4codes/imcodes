...
export const MCP_TOOL_DISCOVERY_REFRESH_INSTRUCTIONS = [
  ...
  'For OCU, Open Computer Use, computer use, or computer control requests, never infer unavailability from the initial callable list: search the exact alias ocu to publish the bounded file-transfer-computer-use group, then use its returned copyable fallbackCalls for computer_use_docs or computer_use_call when either schema is still absent in the current turn.',
