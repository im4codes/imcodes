...
const CJK_ANALYSIS_OPENERS = /^(?:请|帮我|帮忙|麻烦|然后|接着|先|再|并|同时|另外|最后)*(?:分析|调研|研究|总结|解释|说明|查找|搜索|阅读|查看|梳理|对比|比较|列出|列举|理解|定位|排查|看看|看下|看一下|统计|概括|归纳|调查|追踪|找出|找到|了解|回答|解读|描述|汇总|评估原因|只读|仅分析|只分析)/;
